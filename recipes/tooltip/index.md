...
Далее опишем доступные положения тултипа относительно кнопки. Дефолтное – сверху (`position-area: top;`). Если расположить тултип сверху не удалось, попросим браузер отрисовать тултип снизу, слева или справа (` position-try-fallbacks: --bottom, --left, --right;`). И опишем, что под этим подразумеваем.
...
Сделаем тултип якорем. Он будет нужен для управления видимостью хвостиков.
